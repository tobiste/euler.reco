...
The best fit small circle can be retrieved by `euler_reco()`. In our example, the structure represents a strike-slip fault and, thus, expected to follow a small-circle trajectory of the Euler pole to be approximated. Thus, we set the `sc` argument to `TRUE` in order to calculate a small circle fitting the structure:
```{r euler_solution}
...
A good approximation of the structure by the small circle should have a low deviation of the structure from the small circle (`euler_deviation()`). The distributions of the deviations can be evaluated by:
