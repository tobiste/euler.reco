...
The best fit small circle can be retrieved by `euler_solution()`. In our example, the structure represents a strike-slip fault and, thus, expected to follow a small-circle trajectory of the Euler pole to be approximated. Thus, we set the `sm` argument to `TRUE` in order to calculate a small circle fitting the structure:
```{r euler_solution}
res <- euler_solution(tintina, sm = TRUE)
res
```
